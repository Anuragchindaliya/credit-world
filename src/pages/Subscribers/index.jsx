...
const DownloadCSV = () => {
  ...
  const subscribers = useQuery("subs", getAllSubs, {
  ...
  const count = subscribers?.data?.data?.count;

  if(subscribers.isError){
    return <div className="container py-4  d-flex justify-content-center align-items-center h-52">
    <h4 className="text-xl">{subscribers?.error?.response?.data?.message}</h4>
  </div>
  }
  return (
    <>
      ...
      <section className="container">
        {subscribers?.isLoading ? (
          ...
          <TableLayout data={subscribers?.data?.data?.result} columns={columns} />
        )}
      </section>
    </>
  );
